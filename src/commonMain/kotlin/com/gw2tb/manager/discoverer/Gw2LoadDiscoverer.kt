...
class Gw2LoadDiscoverer(
    ...
    private val gw2Load: Gw2Load = Gw2Load(libraryPath)
...
private class Gw2Load(libraryPath: Path) : AutoCloseable {

    private val libraryArena = Arena.ofShared()
    private val symbolLookup = SymbolLookup.libraryLookup(libraryPath, libraryArena)
    ...
    private val GetAddonsInDirectory by lazy(LazyThreadSafetyMode.NONE) {
        ...
        Arena.ofConfined().use { arena ->
            try {
                ...
                return List(size = count) { index ->
                    EnumeratedAddOn.reinterpret(res.asSlice(index * EnumeratedAddOn.LAYOUT.byteSize(), EnumeratedAddOn.LAYOUT.byteSize()))
                }
            } catch (t: Throwable) {
                t.printStackTrace()
